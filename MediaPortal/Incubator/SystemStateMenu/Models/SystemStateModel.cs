    ...
    public static void DoAction(SystemStateAction action)
    {
      switch (action)
      {
        ...
        case SystemStateAction.RestartMP:
          ServiceRegistration.Get<ILogger>().Debug("ShutdownManager: Restart MediaPortal Action has been executed: not implemented, yet");
        ...
        case SystemStateAction.SleepTimer:
          ...
          //  CancelTimer();
          //else
            ServiceRegistration.Get<IWorkflowManager>().NavigatePush(Consts.WF_STATE_ID_SLEEPTIMER_DIALOG);
          return;
      }
    }
